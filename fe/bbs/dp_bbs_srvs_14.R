...
names(svrs14) <- c("tafsil-2h", "tafsil-2p", "tafsil-3", "tafsil-4", "tafsil-9") # Rename lists
...
demo <- ddply(tafsil2p, ~zila, summarise,
              ...
              "prop_pop_rural"=round(sum(area==1)/length(HH1),2),
              "prop_pop_women"=round(sum(Q12==2)/length(HH1),2),
              "pop_>15y"=sum(Q11>15),
